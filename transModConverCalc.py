...
def trans_mod_conver_cal(prior, current):
    sheet = "HOTransModConverCal_Ext"
    columns = ["`Transition Category`", "`Years Since Acquisition`", "`Lower Bound`", "`Trans Mod Level`", "Factor"]
    prior_sheet = pd.read_excel(prior, sheet_name="HOTransModConverCal_Ext", header=9)
    current_sheet = pd.read_excel(current, sheet_name="HOTransModConverCal_Ext", header=9)
