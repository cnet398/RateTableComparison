...
def trans_mod(prior, current):
    sheet = "HOTransMod_Ext"
    columns = ["`Transition Category`", "`Years Since Acquisition`", "`Lower Bound`", "`Trans Mod Level`", "Factor"]
    prior_sheet = pd.read_excel(prior, sheet_name="HOTransMod_Ext", header=9)
    current_sheet = pd.read_excel(current, sheet_name="HOTransMod_Ext", header=9)
